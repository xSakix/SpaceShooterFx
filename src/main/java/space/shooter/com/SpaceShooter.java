...
import space.shooter.com.components.HPBar;
...
public class SpaceShooter extends GameApplication {
    ...
    private void spawnBonus() {
        if (byType(ComponentTypes.BONUS).size() >= 3*geti("num_stations")) {
            return;
        }

        int max = FXGLMath.random(1, 3*geti("num_stations"));
        for (int i = 0; i < max; i++) {
    ...
    protected void initUI() {
        ...
        HPBar hp = new HPBar(false);
        hp.setHeight(10.);
        hp.setWidth(getAppWidth() - 100);
